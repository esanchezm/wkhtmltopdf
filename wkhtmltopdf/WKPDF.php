...
class WKPDF {
    ...
    private $tmp_footer = null;

    private $margin_page = false;
    ...
    public function __construct($cmd) {
        $this->cmd = dirname(__FILE__).'/bin/wkhtmltopdf.'.self::_getCPU();
        if (!file_exists($this->cmd))
            throw new \Exception('WKPDF static executable "' . htmlspecialchars($this->cmd, ENT_QUOTES) . '" was not found.');

        $tmpfile = tempnam(sys_get_temp_dir(), '/html_pdf');
        rename($tmpfile, $tmpfile.'.html'); // wkhtmltopdf requires file extension
        $this->tmp = $tmpfile.'.html';
    }
    ...
    public function set_footer($footer) {
        $tmpfile = tempnam('/tmp/', 'html_footer_pdf');
        rename($tmpfile, $tmpfile.'.html'); // wkhtmltopdf requires file extension
        $this->tmp_footer = $tmpfile.'.html';

        file_put_contents($this->tmp_footer, $footer);
    }


    public function enable_margin_page() {
        $this->margin_page = true;
    }
    ...
    public function render() {
        ...
        if (((int) $this->pdf['return']) > 1 && ((int) $this->pdf['return']) != 2) {
    ...
    public static function download_binary() {
        mkdir(basename(__FILE__).'/bin', 0755);
        var_dump(self::_getCPU());
    }
}
